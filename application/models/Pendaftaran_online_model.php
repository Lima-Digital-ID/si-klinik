...
class Pendaftaran_online_model extends CI_Model
{
  ...
  function insert($data)
  {
    ...
    {
      $this->db->select('dr.nama_dokter, po.id_dokter, po.nama_lengkap, po.nik, po.tanggal_lahir, po.golongan_darah, po.status_menikah, po.pekerjaan, po.alamat, po.kabupaten, po.rt, po.rw, po.nama_orang_tua_atau_istri, po.nomer_telepon, po.tipe_periksa, ps.no_id_pasien, ps.no_rekam_medis');
      $this->db->from('tbl_pendaftaran_online po');
      $this->db->join('tbl_dokter dr', 'po.id_dokter = dr.id_dokter');
      $this->db->join('tbl_pasien ps', 'po.nik = ps.nik');
