...
class Tbl_obat_alkes_bhp_model extends CI_Model
{
	...
	function get_all_obat($id_klinik = null,$json=false)
    {
        ...
        $tipeGetData->select("SUM(tid1.jumlah) - COALESCE((SELECT SUM(tid2.jumlah) as jumlah_stok FROM `tbl_inventory_detail` tid2 JOIN tbl_inventory ti2 ON tid2.id_inventory=ti2.id_inventory WHERE ti2.inv_type='TRX_STUFF' AND ti2.id_klinik=MAX(ti.id_klinik) AND tid2.kode_barang=tid1.kode_barang), 0) - COALESCE((SELECT SUM(tid2.jumlah) as jumlah_stok FROM `tbl_inventory_detail` tid2 JOIN tbl_inventory ti2 ON tid2.id_inventory=ti2.id_inventory WHERE ti2.inv_type='RETURN_MONEY' AND ti2.id_klinik=MAX(ti.id_klinik) AND tid2.kode_barang=tid1.kode_barang), 0) + toa.stok_awal as stok_barang, MAX(tid1.kode_barang) AS kode_barang, MAX(tid1.harga) AS harga, MAX(toa.harga) AS harga_jual, MAX(tid1.diskon) AS diskon, MAX(tid1.tgl_exp) AS tgl_exp, MAX(toa.nama_barang) AS nama_barang");
