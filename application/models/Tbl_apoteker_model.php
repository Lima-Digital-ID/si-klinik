...
class Tbl_apoteker_model extends CI_Model
{

    public $table = 'tbl_apoteker';
    public $id = 'id_apoteker';
    public $order = 'DESC';
    ...
    // datatables
    function json() {
        $this->datatables->select('tbl_apoteker.*, tbl_apoteker.id_apoteker, coalesce(tbl_klinik.nama, "-") as nama_klinik');
        $this->datatables->from('tbl_apoteker');
        $this->datatables->join('tbl_user','tbl_apoteker.id_apoteker=tbl_user.id_apoteker', 'left');
        $this->datatables->join('tbl_klinik','tbl_user.id_klinik=tbl_klinik.id_klinik','left');
        $this->datatables->add_column('action', anchor(site_url('apoteker/update/$1'),'<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', array('class' => 'btn btn-success btn-sm'))." 
                ".anchor(site_url('apoteker/delete/$1'),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-sm" onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id_apoteker');
