...
class Tbl_rapid_antigen_model extends CI_Model
{
    ...
    {
        $this->db->select('no_sampel');
        $this->db->where(['MONTH(tgl_buat)' => date('m'), 'YEAR(tgl_buat)' => date('Y'),'tipe' => $code]);
        $this->db->order_by('id_rapid','desc');
        $this->db->limit(1);
        return $this->db->get($this->table)->result();
    }
    public function listPemeriksaanDokter($code)
    {
        $this->datatables->select('id_rapid,no_sampel,nama,nik_or_passport');
        $this->datatables->from($this->table);
        $this->datatables->where(['status' => '0','tipe' => $code]);
    ...
    public function jsonRapid($id_klinik,$status)
    {
        ...
        if($status=='0'){
            ...
            $this->datatables->where(['r.tgl_pemeriksaan' => date('m')]);
